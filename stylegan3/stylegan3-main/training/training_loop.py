...
def training_loop(
    ...
    if resume_pkl is not None:
        if rank == 0:
            print(f'Loading networks directly from "{resume_pkl}"')
        ...
        if G.img_resolution != training_set.resolution:
            raise RuntimeError(
                f'Resume G resolution {G.img_resolution} does not match '
                f'dataset resolution {training_set.resolution}'
            )
        if G.img_channels != training_set.num_channels:
            raise RuntimeError(
                f'Resume G channels {G.img_channels} does not match '
                f'dataset channels {training_set.num_channels}'
            )
        if G.c_dim != training_set.label_dim:
            raise RuntimeError(
                f'Resume G c_dim {G.c_dim} does not match '
                f'dataset label_dim {training_set.label_dim}'
            )

        if hasattr(D, 'img_resolution') and D.img_resolution != training_set.resolution:
            raise RuntimeError(
                f'Resume D resolution {D.img_resolution} does not match '
                f'dataset resolution {training_set.resolution}'
            )
        if hasattr(D, 'img_channels') and D.img_channels != training_set.num_channels:
            raise RuntimeError(
                f'Resume D channels {D.img_channels} does not match '
                f'dataset channels {training_set.num_channels}'
            )
        if hasattr(D, 'c_dim') and D.c_dim != training_set.label_dim:
            raise RuntimeError(
                f'Resume D c_dim {D.c_dim} does not match '
                f'dataset label_dim {training_set.label_dim}'
            )
    ...
    while True:
        ...
        # Print loss/stat values every tick.
        if rank == 0:
            def get_stat(name):
                stat = stats_dict.get(name, None)
                return None if stat is None else float(stat.mean)
            ...
            loss_items = [
                ...
                ('D_reg', 'Loss/D/reg'),
                ('R1', 'Loss/r1_penalty'),
                ('real_score', 'Loss/scores/real'),
                ('fake_score', 'Loss/scores/fake'),
                ('real_sign', 'Loss/signs/real'),
                ('fake_sign', 'Loss/signs/fake'),
            ]

            loss_fields = []
            for short_name, stat_name in loss_items:
                value = get_stat(stat_name)
                if value is not None:
                    loss_fields.append(f'{short_name} {value:.4g}')

            if len(loss_fields) > 0:
                print('loss ' + '  '.join(loss_fields))
